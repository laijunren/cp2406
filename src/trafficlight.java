...
public class TrafficLight {
    ...
    private static final String TRAFFICLIGHT_ID_PREFIX = "light_";
    private String id;
    ...
    public TrafficLight(String id, Road road) {
        this.id = TRAFFICLIGHT_ID_PREFIX + id;
        state = RED;
...
//        System.out.printf("%s is:%s on %s at position:%s%n", this.getId(), this.getState(), this.getRoadAttachedTo().getId(), this.getPosition());
    ...
    public String toString() {
        return String.format("%s is:%s on %s at position:%s%n",
                this.getId(), this.getState(), this.getRoadAttachedTo().getId(), this.getPosition());
    }
}
