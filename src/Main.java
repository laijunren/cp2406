...
//            int speedLimitInput = simController.nextInt();
            ...
            Road road = new Road(Integer.toString(i), speedLimitInput, lengthInput, new Location(0, 0));
            roads.add(road);
        }
        System.out.println("\nRoads;");
        for (Road road : roads) {
        ...
        for (int i = 0; i < carSpawns; i++) {
            Car car = new Car(Integer.toString(i), roads.get(0));
        ...
        for (int i = 0; i < lightSpawns; i++) {
            TrafficLight trafficLight = new TrafficLight(Integer.toString(i), roads.get(0));
            lights.add(trafficLight); // all created lights will begin on road_0.
        ...
        roads.get(1).setStartLocation(new Location(roads.get(0).getLength() + 1, 0)); // place road_1 to a position at the end of road_0.
